        ...
        <View style={[styles.container, { backgroundColor: theme.colors.bg }]}>
            <View style={[styles.statusCard, { backgroundColor: theme.colors.card }]}>
                <View style={styles.statusRow}>
                    <Text style={[styles.statusLabel, { color: theme.colors.textSecondary }]}>Conexion:</Text>
                    <OfflineBadge />
                </View>
                <View style={styles.statusRow}>
                    <Text style={[styles.statusLabel, { color: theme.colors.textSecondary }]}>Items pendientes:</Text>
                    <Text style={[styles.statusValue, { color: theme.colors.text }, pendingCount > 0 && { color: theme.colors.warning }]}>{pendingCount}</Text>
                </View>
                <View style={styles.statusRow}>
                    <Text style={[styles.statusLabel, { color: theme.colors.textSecondary }]}>Conflictos:</Text>
                    <Text style={[styles.statusValue, { color: theme.colors.text }, conflictCount > 0 && { color: theme.colors.error }]}>{conflictCount}</Text>
                </View>
                <View style={styles.statusRow}>
                    <Text style={[styles.statusLabel, { color: theme.colors.textSecondary }]}>Ultima sincronizacion:</Text>
            ...
            <View style={styles.queueHeader}>
                <Text style={[styles.queueTitle, { color: theme.colors.text }]}>Cola de Sincronizacion</Text>
            </View>
            ...
            {loading ? (
                ...
                <FlatList
                    ...
                    renderItem={({ item }) => (
                        <View style={[styles.queueItem, { backgroundColor: theme.colors.card }]}>
                            <Text style={styles.queueIcon}>{getStatusIcon(item.status)}</Text>
                            <View style={styles.queueInfo}>
                                <Text style={[styles.queueEntity, { color: theme.colors.text }]}>{item.entity}</Text>
                                <Text style={[styles.queueOp, { color: theme.colors.textSecondary }]}>{item.operation} - {item.entity_id?.slice(0, 8)}</Text>
                            </View>
