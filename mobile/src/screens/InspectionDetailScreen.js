...
const InspectionDetailScreen = ({ route, navigation }) => {
    ...
    if (!inspection) {
        return (
            <View style={styles.center}>
                <Text style={styles.emptyText}>Inspeccion no encontrada</Text>
            </View>
        );
    }
    ...
    return (
        <ScrollView style={styles.container}>
            ...
            <View style={styles.infoCard}>
                ...
                <InfoRow label="Direccion" value={inspection.address} />
                <InfoRow label="Tipo" value={inspection.inspectionType} />
                <InfoRow label="Fecha" value={inspection.scheduledDate ? new Date(inspection.scheduledDate).toLocaleDateString('es-PE') : '-'} />
                <InfoRow label="Inspector" value={inspection.inspector ? `${inspection.inspector.firstName} ${inspection.inspector.lastName}` : '-'} />
                {inspection.is_dirty && (
                    <View style={styles.dirtyBadge}>
                        <Text style={styles.dirtyText}>Cambios sin sincronizar</Text>
                    </View>
                )}
            </View>

            <View style={styles.section}>
                <Text style={styles.sectionTitle}>Areas ({areas.length})</Text>
                {areas.length === 0 ? (
                    <Text style={styles.emptySection}>Sin areas registradas</Text>
                ) : (
                    areas.map((area) => (
                        <View key={area.id} style={styles.listItem}>
                            <Text style={styles.listItemTitle}>{area.name}</Text>
                            <Text style={styles.listItemSub}>{area.category} - {area.calculatedAreaM2 ? `${area.calculatedAreaM2} m2` : 'Sin dimensiones'}</Text>
                        </View>
                    ))
                )}
            </View>
            ...
            <View style={styles.section}>
                ...
                {observations.length === 0 ? (
                    <Text style={styles.emptySection}>Sin observaciones</Text>
                ) : (
                    observations.map((obs) => (
                        <View key={obs.id} style={styles.listItem}>
                            <Text style={styles.listItemTitle}>{obs.title}</Text>
                            <Text style={styles.listItemSub}>{obs.severity} - {obs.type}</Text>
                        </View>
                    ))
                )}
            </View>
            ...
            <View style={styles.section}>
                ...
                {photos.length === 0 ? (
                    <Text style={styles.emptySection}>Sin fotos</Text>
                ) : (
                    <Text style={styles.emptySection}>{photos.filter(p => p.uploadStatus === 'uploaded').length} subidas, {photos.filter(p => p.uploadStatus === 'pending').length} pendientes</Text>
                )}
            </View>
