...
const { createUserSchema, updateUserSchema, toggleStatusSchema } = require('../validators/userValidator');
...
// GET /api/v1/users/stats - Estadisticas (solo admin)
...
router.post('/', authorize('admin'), validateJoi(createUserSchema), userController.createUser);

// GET /api/v1/users/:id - Obtener usuario por ID (solo admin)
router.get('/:id', authorize('admin'), userController.getUserById);

// PUT /api/v1/users/:id - Actualizar usuario (solo admin)
router.put('/:id', authorize('admin'), validateJoi(updateUserSchema), userController.updateUser);
...
router.patch('/:id/status', authorize('admin'), validateJoi(toggleStatusSchema, 'body'), userController.toggleUserStatus);
